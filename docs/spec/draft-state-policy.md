...
`schemas/process-flow.schema.json` / `schemas/extensions-*.schema.json` / `schemas/conventions.schema.json` 等のグローバル定義 schema は、最終形の品質ゲートとして扱う。draft-state を保存したいという理由で schema を緩めてはならない。
...
- error: View ID が空、View ID が重複している、画面定義 JSON と screen-items の参照関係が壊れている
- warning: 画面名が仮置き、説明が未入力、主要項目の表示ラベルが未確定

Table の代表例:

- error: Table ID が空、Table ID が重複している、Column ID が重複している、主キー参照が存在しない
- warning: 論理名が未入力、説明が未入力、業務的な必須カラム候補が未確定

ProcessFlow の代表例:

- error: step ID が重複している、参照先 step が存在しない、必須 action 定義が欠落している、下流モードで `draft` が残っている
- warning: outcome が未定義、note に仮置きが残っている、業務例外の扱いが未確認、成熟度が `provisional` のまま
...
3. 実行時 UI に AJV を入れると bundle size と依存関係が増える
...
- `designer/src/components/common/MaturityBadge.tsx`
- `designer/src/styles/validation.css`
- `designer/src/validation/viewValidation.ts`
- `designer/src/validation/tableValidation.ts`
- `designer/src/validation/processFlowValidation.ts`
