...
```ts
// 旧形式 (string) は assign 相当
// 新形式 (object) は operation で代入方式を指定可能 (#253)
...
interface OutputBindingObject {
  ...
  initialValue?: string;              // accumulate: "0", push: "[]" 相当
}

type OutputBinding = string | OutputBindingObject;

interface StepBase {
  outputBinding?: OutputBinding;  // 例: "users" / { name: "subtotal", operation: "accumulate" }
}
```
...
| `StepBase` | `outputBinding?: string \| OutputBindingObject` | 新規 (string は assign 相当、object は operation 指定可) |
...
- 2026-04-24: v1.0 凍結。§3.2 OutputBinding 型を実装と合わせ string | OutputBindingObject に更新 (#253)
