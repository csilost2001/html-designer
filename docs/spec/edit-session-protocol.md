...
| **editSessionId** | EditSession の identifier。EditSession の lifecycle と一致する識別子。`editSession.create` 時に発番、`editSession.transferEdit` 時は維持、`editSession.discard` / `editSession.detach` 時に終了。 |
...
`editSessionStore.transferEdit(fromSessionId, toSessionId, editSessionId)` の前提:

1. `toSessionId` が `View` participant として既に登録されていること (= 連絡を取った証拠)
2. 同一 mutex / critical section 内で role 変更を実行
3. broadcast `editSession.roleChanged` op:`transferred` で全 participant に通知
...
| `lock.forceRelease` | 削除 (Phase 6: `editSession.detach` forced=true 経路に統合) |
...
- `EditSessionBadge.tsx` / `PresenceBadge.tsx` — そのまま継続 (旧 `SessionBadge.tsx` は Phase 6 で削除)
- `TransferNotificationBanner.tsx` — `editSession.roleChanged` event listen のみ (旧 `lock.changed` listen は Phase 6 で削除)
...
- [x] § 3 EditSession 概念の正規実装
- [x] § 5 ライフサイクル 1-6 step が動作 (vitest + e2e)
- [x] § 7 take-over の atomicity (View → Edit / Edit → View が同一 critical section)
- [x] § 8 save 規則 (Edit のみ可 / 全員 View なら誰でも可、audit log 必須)
- [x] § 9 複数 EditSession 並存 + last-save-wins 警告ダイアログ
- [x] § 10 AI participant + `Alice@AI` 表示 + `parentHumanSessionId` 保持
- [x] § 11 URL `?session=<editSessionId>` 経由の attach
- [x] § 12 TTL 2 段階 (Active → Discarded → 完全削除) の自動削除
- [x] § 13 attach 時の initial fetch (memory store から最新 payload 取得) — § 1.1 根本欠陥の解消
- [x] § 14 broadcast プロトコルの新 event 一式
- [x] PR #888 の旧 API (lock / draft 系) を全削除 (§ 15.1 表参照)
- [x] 既存 spec (`collab-presence.md` / `edit-session-draft.md`) からの参照リンク追加
...
6. 旧 API 完全削除 + 既存 spec 改訂 (Phase 6) ✅ 完了 (#903)
7. テスト + e2e (Phase 7)
...
| 2026-05-07 | Phase 6 (#903): 旧 API 完全削除 (lock.* / draft.* MCP tools / handlers / lockManager.ts / draftStore.ts / onBehalfOfSession.ts 削除)、useEditSessionLegacy 削除、SessionBadge.tsx 削除、TransferNotificationBanner の lock.changed listen 削除、useDraftRegistry を editSession.list ベースに移行。§15.2 UI components 欄・§15.1 互換性表を更新。|
