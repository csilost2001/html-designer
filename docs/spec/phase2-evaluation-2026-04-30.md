...
> **📝 ステータス**: 完了済 **historical record** (Phase 2 統合評価レポート、#493、Phase 3 移行判断を含む)。本書は当時の評価記録として保持、削除不可 (他 spec から参照ある場合あり)。最新の状況・運用は現行 spec を参照。
...
**成果物**: `ai-skills/create-flow/SKILL.md` 更新
...
**成果物**: `ai-skills/review-flow/SKILL.md` 更新 + `validate-dogfood.ts` に `--flow <path>` 引数追加
...
6. **Skill governance**: schema (`schemas/v3/*.json`) は AI 権限外、実装 (`frontend/src/schemas/*.ts`) と切り分け (#511 ガバナンス)
