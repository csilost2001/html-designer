...
| `branch` | `branches` (各 branch に `id` / `condition` / `steps`) | 条件分岐 |
| `loop` | `loopKind`, `steps` | 繰り返し (variant により追加 field あり) |
...
| `audit` | `action` | 監査ログ |
| `workflow` | `pattern`, `approvers` (+ variant により `escalateAfter`/`escalateTo`/`quorum` 等) | 承認パターン |
| `transactionScope` | `steps` | TX 境界明示 |
| `eventPublish` | `topic` | イベント発行 |
| `eventSubscribe` | `topic` | イベント購読 |
| `closing` | `period` | 締め処理 (月次/四半期等) |
| `cdc` | `tableIds`, `captureMode`, `destination` | CDC (変更データキャプチャ) |
| `aiCall` | `modelRef`, `messages` | AI 呼び出し |
| `aiAgent` | `modelRef`, `messages`, `tools` | AI agent (tools 必須) |
| `extension` | (`kind` が `namespace:name` パターン、variant 別) | プラグイン定義の step |

**重要**: 本 cheatsheet は schema を機械抽出した snapshot。schema 更新時は `.tmp/extract-step-required.mjs` 相当のスクリプトで再生成すべき。spec 編集者の記憶に頼って書き換えない。不確実な場合は必ず `schemas/v3/process-flow.v3.schema.json` の該当 `$defs/<XxxStep>` を Read で確認。
...
- `examples/<project>/<dataDir>/generic-definitions/<kind>/*.json` 配下の出力 (data-contract / domain-type / exception-type / application-rule / ui-behavior / runtime-policy / component-definition / ui-fragment) — 現行 loader は読まないが、将来 schema 確定時の取り込み用に保存
