...
**root 構造** — `meta` + `context` + `actions` + `authoring` の 4 セクション。`kind`/`id`/`code`/`name` を root に直書きは AJV 落ち:
```jsonc
{ "meta": { "id", "name", "kind", "createdAt", "updatedAt", ... }, "context": { "catalogs": { "errors": {...}, "events": {...} } }, "actions": [...] }
```
...
- ✅ 現行 schema では binding 情報を `description` 内に **binding grammar v1** で退避する (§3.1 / §0.5 参照)。grammar:
  - sentinel `[binding.v1]` 必須
  - 続けて `<key>=<value>; <key>=<value>; ...` のセミコロン+空白区切り
  - 標準 key: `binding.attr` (HTML 属性、`:` 許容) / `binding.path` / `binding.role` / `binding.formatHint` / `source` / `note`
  - value 内に `=` / `;` を含めない
