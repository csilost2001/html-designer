...
> **⚠ 部分 Obsolete (Phase 6 / #903 完了 2026-05-07):** §4 のメッセージシーケンス・状態管理 (§4.2 以降) は edit-session-protocol.md に統合済。**§4.1 (責務境界)** は frontend (`usePresenceRegistry.ts`) から現役参照中、有効。
...
> **⚠ Obsolete (Phase 6 / #903 完了 2026-05-07):** 本節は edit-session-protocol.md で置き換え済。frontend コード参照は §9 (Activity taxonomy) / §6 (Heartbeat 仕様) のみ有効。
