...
- メタ ISSUE: [#793](https://github.com/csilost2001/harmony/issues/793) (project 単位設計の元起票) + 2026-05-05 マルチエディタ対応メタ ISSUE (画面単位化を含む統合 PR)
...
`frontend/src/styles/common.css:1-23` (抜粋):
...
`frontend/src/components/Designer.tsx:88-93` で `THEME_URLS` 経由で iframe にテーマ CSS を注入する仕組み実装済 (standard / card / compact / dark)。

**実装の負債 (Bootstrap class ハードコード)**:

`frontend/src/grapes/blocks.ts:35-47` (抜粋):
...
2. `project.techStack.designer.cssFramework` (なければ project default、#826 で `project.design` から移行)
...
│   2. harmony.json       design.cssFramework (default)   │
│   3. 最終 default       "bootstrap"                     │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
...
`frontend/src/styles/themes/` (新規ディレクトリ) に:
...
1. `frontend/package.json` に `tailwindcss` / `postcss` / `autoprefixer` を依存追加
2. `frontend/postcss.config.js` 作成
3. `frontend/tailwind.config.ts` 作成 (content scan path に `themes/theme-tailwind.css` を含める)
...
#### 2.4.1 `schemas/v3/harmony.v3.schema.json` (project default)
  ...
  "ScreenDesign": {
    ...
    "properties": {
      "cssFramework": {
        "type": "string",
        "enum": ["bootstrap", "tailwind"],
        "description": "本画面の CSS フレームワーク。画面作成時に固定、以降変更不可。省略時は project.techStack.designer.cssFramework を参照、それも省略なら 'bootstrap'"
      }
    }
  }
}
...
現状 (`frontend/src/components/Designer.tsx`):
...
const cssFramework =
  ...
  ?? project.techStack?.designer?.cssFramework  // #826: project.design から移行
  ?? "bootstrap";
...
- [x] `schemas/v3/harmony.v3.schema.json` に `design.cssFramework` field 追加
- [x] `frontend/src/types/` に `CssFramework` 型定義追加
- [x] `frontend/src/grapes/blocks.ts` から Bootstrap / common.css ハードコード排除 (semantic class のみ出力)
- [x] ブロック数 60+ → 15-20 個程度に絞り込み
- [x] `frontend/src/styles/themes/theme-bootstrap.css` + `theme-tailwind.css` 新規作成
- [x] `frontend/package.json` に Tailwind 関連依存追加、`postcss.config.js` / `tailwind.config.ts` 作成
- [x] `Designer.tsx` が `project.techStack.designer.cssFramework` を読み取り、対応 theme CSS を canvas iframe に注入 (#826 で `project.design` から移行)
- [x] 既存 3 サンプル (retail / realestate / english-learning) で `cssFramework: "bootstrap"` 設定後、regression なし

### 6.2 画面単位化 (本改訂、マルチエディタ対応シリーズで対応)

- [x] `schemas/v3/screen.v3.schema.json` の `ScreenDesign` に `cssFramework` field 追加 (画面側 override、optional)
- [x] `Designer.tsx` の cssFramework 解決ロジックを画面単位化 (`screen.design.cssFramework` ?? `project.techStack.designer.cssFramework` ?? `"bootstrap"`) (#826)
- [x] 既存 3 サンプルが project default で全画面 bootstrap を継続適用すること (regression なし)
- [x] 画面単位混在 dogfood: 1 プロジェクトで bootstrap 画面と tailwind 画面が共存、各々が canvas iframe で正しい theme をロードすることを E2E で検証
- [x] dogfood report 作成 (マルチエディタ対応シリーズの dogfood に統合)
