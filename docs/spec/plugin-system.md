...
```
examples/<project-id>/extensions/  ← 各 project 内の拡張定義サンプル (namespace.v3.json 形式)
  steps.json
  field-types.json
  ...
...
- **`examples/<project-id>/extensions/`**: 各 project 内の拡張定義サンプル (namespace.v3.json 形式)。GM50 を含む業界別テンプレートをここに置く
- **`data/extensions/`**: 実行時にバリデーターが読む場所。本リポジトリでは追跡対象 (CLAUDE.md の `data/ is gitignored` 記述は古く実態と異なる。`data/process-flows/` 等と同様に追跡する)
- 両者の関係は将来 `harmony.json` の `extensionsPath` 設定で切り替え可能にする (将来拡張)
...
- 外部パス設定: `harmony.json` に `extensionsPath` を追加し、`data/extensions/` 以外の場所を指定できるようにする
...
| サンプル | `docs/sample-project/process-flows/cccccccc-{0005,0006,0007,0008}-*.json` | `typeCatalog` セクションを抽出し `docs/sample-project/extensions/response-types.json` に移行 (#445/#447 で実施済み、現在は examples/<project-id>/ を使用) |
