...
| 概念 | 場所 | axis | 役割 |
|---|---|---|---|
| **ViewDefinition** (本書) | `view-definitions/<id>.json` | 画面 (UI viewer) | 1 viewer = 1 source、列を画面表示型 (FieldType) で表現。`Screen.viewDefinitionRefs[]` で 1:N 参照される |
...
**注意**: parameterRefs は静的な型宣言のみ。**画面の input 項目から `@param.*` への動的 binding 方式は未確定** (#762 で別途仕様化中)。
...
`Screen.viewDefinitionRefs: ViewDefinitionId[]` で 1:N 参照する。
...
// screens/<id>.json
{
  "id": "<screen-uuid>",
  "kind": "list",
  "items": [],   // input 項目はここに埋め込み (screen-items.md 参照)
  "viewDefinitionRefs": [
    "<view-definition-uuid>"
  ]
}
```

**1:N で複数指定する典型**: `kind="list"` の画面で、タブ切替や絞込パネル切替で複数 viewer を提示する場合 (例: 「全件 / アクティブのみ / 廃番含む」をそれぞれ別 ViewDefinition に分け、画面が切替制御を持つ)。

**画面項目 (items[]) との関係**: items[] は input フォーム要素、`viewDefinitionRefs[]` は配列データの表示部。同じ画面内で input × N + viewer × M を同居させる構成が一般的 (検索画面 / 一覧画面の組合せ)。

`kind="list"` の Screen で `items: []` (input 0 件) かつ `viewDefinitionRefs: []` (viewer 0 件) の場合、`runtimeContractValidator [EMPTY_SCREEN_ITEMS]` が warning を発報する (画面が空白なため、空白意図ならコメントを残すか、設計途中なら maturity=draft で許容)。
...
- [`screen-items.md`](screen-items.md) — 画面項目定義 (`Screen.items[]` の input/output 項目、`viewDefinitionRefs` と同居)
...
- **#762**: 検索フォーム input → ViewDefinition `filterDefaults` / `parameterRefs` への動的 bind 方式 — 「検索フォーム + 一覧表示」 業務 UI で最頻出の連携パターン。本書は静的 `filterDefaults` までを扱う
- **#763**: design.json の preview-only ダミーデータ vs 実バインディングのマーキング規約 — `viewDefinitionRefs` で一覧バインディングしている場合に design.json 内のサンプル行をどう扱うか、関連話題
...
- ViewDefinition と ProcessFlow output の binding (画面 ロード時に flow を起動 → 結果を viewer に渡すパターン、#762 と関連)
