...
### Phase 2-B — `/generate-tests` skill 拡張 (完了、本 PR)

- `.claude/skills/generate-tests/SKILL.md` の AI flow 検出ロジックを `kind=externalSystem` + `systemRef=claudeApi` から **`kind ∈ {aiCall, aiAgent}`** に更新
- responseFormat 別 (`text` / `json` / `structuredObject` / `streaming`) の mock 戦略を P5-3 に整理
  - mock helper は provider 中立 (`mocks/ai-runtime.ts` の `mockAiText` / `mockAiStructured` / `mockAiJson` / `mockAiStreaming` / `mockAiFailure` / `mockAiFormatViolation` / `mockAiWithToolCalls`)
  - 戻り値は spec §「outputBinding の値構造」の正規化形式 (`{ text?, object?, raw?, finishReason?, usage?, toolCalls? }`)
- AI-3 観点を responseFormat-aware に再設計 (text / streaming は skip、json / structuredObject は schema 違反 → 502)
- AI-4 観点を outcomes.failure 駆動に再設計 (failure responseId → status 自動解決)
- AiMessageSpread (`{kind:"spread", ref}`) の検出と fixture 化を P5-4 に追加
- AiImageSource variant (`fileRef` / `url(literal)` / `url(@-expression)` / `base64`) 別の fixture 戦略を P5-5 に追加
- modelEndpoints catalog 解決を project + flow merge に対応 (`harmony/catalogs/external.json` ⊕ `<flow>.context.catalogs.modelEndpoints`)
- 旧 `mocks/claude-api.ts` (Anthropic 形式 HTTP mock) は廃止 → `mocks/ai-runtime.ts` (provider 中立) に置換
- mock target (`AI_RUNTIME_SERVICE_TYPE` / `_METHOD` / `_IMPORT_PATH`) は Phase 2-C で確定する PLACEHOLDER として記録
- smoke 確認: diary 4 flow (f7a8b9c0 / a9b0c1d2 / b0c1d2e3 / c1d2e3f4) + english-learning(+tailwind) 96118ae1 で skill 経由の生成パスを mental trace で検証済 (validate:samples 27/27 pass、regression なし)
