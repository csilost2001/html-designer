...
VS Code Dev Containers は user 設定 `dotfiles.repository` が指定されていれば rebuild 時に dotfiles repo を `~/dotfiles/` に clone する。ただし `install.sh` の自動実行は user 個別の `dotfiles.installCommand` 設定依存で取りこぼしやすいため、本リポジトリでは **`postCreateCommand` 末尾で必ず実行** する形にしている:

```jsonc
"postCreateCommand": "... && ([ -x ~/dotfiles/install.sh ] && bash ~/dotfiles/install.sh || true)"
```

- dotfiles repo を持つ開発者: rebuild 毎に `install.sh` が走り、symlink (`~/.bash_aliases` 等) が再作成される
...
ベース image の `~/.bashrc` には標準で `[ -f ~/.bash_aliases ] && . ~/.bash_aliases` が含まれているので、symlink さえ作れば alias は次のシェル起動時から効く。
