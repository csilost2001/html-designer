...
これらは bind mount で container 内 `/home/node/.{claude,codex,harmony}/` と双方向同期され、**rebuild しても永続化されます**。backup したい場合は通常の `cp -r` / `rsync` / `git` で WSL2 host から直接触れます。
...
3. **(任意) 個人 alias (`ccd` / `cdx` 等)**: VS Code user settings.json に dotfiles 3 行を追加。詳細は本書「個人 alias」節を参照。
...
```
~/.agent-containers/                    ← AI agent (claude / codex 等) container 用データ root
...
- 結果: host 側に `~/.agent-containers/harmony/.claude/`、`~/.agent-containers/harmony/.codex/`、`~/.agent-containers/harmony/.harmony/` のディレクトリ群が作られ、container 内 `/home/node/.{claude,codex,harmony}/` と双方向同期される
