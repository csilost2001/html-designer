...
| 項目 | 期間目安 |
|---|---|
| **Git / VSCode / DevContainers** (利用者) | まとめて 2 週間程度 (経験者多く採用面でも有利) |
| **MD / JSON 理解** ← 本日のメインテーマ | ツール利用で 2-4 週間 / 直接編集なら数ヶ月 |
| **AI エージェント運用** | 基盤整備 (SKILL.md / AGENTS.md) で個人差を埋め 2 週間程度 |
| **整備担当者** (各種ツール導入) | 別途、それぞれ数週間〜数ヶ月 |

</div>
<div>

### 継続リスク (人材・統制面に集約)
- 今回登場した技術 (設計書ツール / AI / コンテナ) に詳しい理解者が **プロジェクトに 1 名以上いるか** で立ち上がりに顕著な差
- サポート役の存在で安心:
  - 設計書ツールの機能追加・改善対応
  - LLM モデル選定の知識
  - **AI エージェントが何をしているかを把握・統制**
- 外部調達人材は GitHub / VSCode / DevContainers の経験者が比較的多い
...
<h1 data-eyebrow="07 / Risks">AI 時代特有のリスクとその対応</h1>

<div class="cols">
<div>

### リスク
- **セキュリティ (プロンプトインジェクション)**: 外部入力 (Issue / 顧客テキスト等) に悪意ある指示が混入 → AI が実行
- **環境破壊**: AI の意図しないファイル削除・コード改変
- **コスト超過 (Copilot 使用時)**: 上位モデルの multiplier が高く (Opus 4.7 x15、GPT 5.5 x7.5 [^16])、無自覚利用で credit 消費が想定を大幅に上回る

</div>
<div>

### その対応
- **環境隔離**: DevContainers / コンテナ隔離 (sandbox 相当) で AI 権限を最小化、機密リポジトリへのアクセスも制限
- **入力の信頼性管理**: 外部入力を信頼レベル付きで扱い、AI 権限境界を明示
- **LLM モデルの使い分け**: Copilot 高 multiplier モデルは必要時のみ、Claude Code / Codex 等を併用してコスト分散

</div>
</div>

<div class="risk">
<strong>AI 時代特有のリスクは従来のセキュリティ・コスト管理の延長線では捉えきれない。</strong>
<small>環境隔離・入力管理・モデル使い分けの 3 点セットを設計時から組み込むのが現実解。</small>
</div>
...
<div class="honest-note">
以上、調査報告および技術選択肢の整理として。記述時点: 2026 年 5 月 19 日、Draft 20260519。
</div>
