...
A: 画面デザイナーで一度フォーム要素をドロップして保存すると、active workspace の `<dataDir>/screen-items/<id>.json` が生成されます (例: `workspaces/<wsId>/<dataDir>/screen-items/<id>.json`)。
...
> **ボタンが無効の場合**: `claude login` 未実行または `backend` 未起動です。
