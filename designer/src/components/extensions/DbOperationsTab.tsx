...
function fileOf(bundle: ExtensionTabProps["bundle"]) {
  const raw = bundle.dbOperations;
  if (raw && typeof raw === "object" && !Array.isArray(raw)) return raw as { namespace?: string; dbOperations?: Row[] };
  return { namespace: "", dbOperations: [] };
}

export function DbOperationsTab({ bundle, saving, onSave }: ExtensionTabProps) {
  ...
  return (
    <div>
      <div className="row g-2 align-items-end mb-3">
        ...
        <div className="col-md-auto"><button className="btn btn-primary btn-sm" disabled={saving} onClick={() => void onSave("dbOperations", { namespace, dbOperations: rows.filter((r) => r.value.trim() && r.label.trim()) })}>保存</button></div>
      </div>
      {rows.map((row, index) => (
