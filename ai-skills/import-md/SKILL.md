...
- ScreenItem: **#1065 で `binding` サブ field 導入済**。新規 import は structured `binding: {kind, path, role, formatHint, sourceNote}` を直接生成 (§3.1 ✅)。MD 内の binding 情報が抽出可能なら structured 形式で生成。抽出曖昧時は legacy `[binding.v1]` description sentinel 形式で一旦退避し、後で `scripts/migrate-binding-v1-to-structured.mjs --apply` で migrate (fallback path)。自由文埋もれは禁止 — migration script が抽出不能になるため。詳細 spec §3.1 / §8.2
- SQL: alias 必須化
