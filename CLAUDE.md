...
|------|-----------|---------|---------------|
| `/` | FlowEditor | Screen flow diagram (ReactFlow) | ❌ route only |
| `/design/:screenId` | ScreenDesigner | WYSIWYG canvas (GrapesJS) | ✅ |
| `/tables` | TableListView | Table definitions list | ❌ route only |
| `/tables/:tableId` | TableEditor | Table column editor | ✅ |
| `/er` | ErDiagram | ER diagram | ❌ route only |
| `/actions` | ActionListView | Action groups list | ❌ route only |
| `/actions/:actionGroupId` | ActionEditor | Action group editor | ✅ |

### Tab policy

**Tabs are for per-resource editors where multiple instances may be open at once.** Navigation and singleton workspaces use plain routing (no tab).

- ✅ タブ化: `Designer` / `TableEditor` / `ActionEditor` — 複数リソースを並行編集する
- ❌ タブ化しない: `FlowEditor` / `TableListView` / `ActionListView` / `ErDiagram` — シングルトンまたはナビゲーション一覧

参考: VS Code / Figma でも同じ区別（ファイルツリー＝パネル、開いたファイル＝タブ）。
