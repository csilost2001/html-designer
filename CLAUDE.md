...
- **AI browser smoke**: `.mcp.json` で Playwright / chrome-devtools MCP を bundled chromium で起動するよう設定済 (2026-05-17、PR #1138)。preview server 立て→`mcp__playwright__browser_navigate` で smoke 可。設定変更後は claude code 再起動必要 (memory `feedback_browser_smoke_headless_chrome_devtools.md`)
