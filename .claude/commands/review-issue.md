---
description: 明示的に `/review-issue <ISSUE番号>` コマンドで呼ばれた時のみ起動する ISSUE 完了監査。通常の会話 (「レビュー」「確認」「完了チェック」等) では起動しない。紐づく全 PR がマージ済みでない段階では起動しない。PR 単位の独立レビューは `/review-pr` を使用し、本コマンドと混同しない。既存レビューへの対応は `gh issue view <N> --comments` を読む別ワークフローであり、本コマンドとは無関係。
...
<!--
  ...
  ⚠️ 起動禁止ケース (重要、誤発動防止):
    - ユーザーが「レビュー指摘を確認」「レビュー結果に対応」「確認して」等と言った時は起動しない。
      それは既存の /review-issue / /review-pr 結果への対応という別ワークフロー。
    - 紐づく PR に未マージのものがある段階では起動しない (本コマンドは全 PR マージ後の事後監査)。
    - `/review-pr` (PR 単位のコード品質レビュー) と混同しない。本コマンドは ISSUE 単位の**網羅性**監査。
    - description のキーワード (「レビュー」「確認」等) がユーザー発言と一致しても、
      **ユーザーが `/review-issue <N>` と明示コマンドで呼ばない限り起動しない**。キーワード粗マッチは誤発動の元。
-->
