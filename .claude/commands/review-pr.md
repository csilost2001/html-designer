---
description: 明示的に `/review-pr <PR番号>` コマンドで呼ばれた時のみ起動する PR 独立レビュー。通常の会話 (「レビューして」「レビュー指摘を確認」「レビュー結果に対応」「レビューを踏まえて修正」等) では起動しない。実装担当セッションでの自己発動も禁止 (本コマンドは別セッション独立レビュー前提)。既存レビューコメントへの対応は `gh pr view <N> --comments` で読む別ワークフローであり、本コマンドとは無関係。
argument-hint: <PR 番号>
---
...
<!--
  ...
  ⚠️ 起動禁止ケース (重要、誤発動防止):
    - ユーザーが「レビュー指摘を確認」「レビュー結果に対応」「レビューを踏まえて修正」等と言った時は起動しない。
      それは **既存の /review-pr 結果への対応** という別ワークフロー。`gh pr view <N> --comments` で既存コメントを読み、
      Must-fix / Should-fix を修正する作業であり、本コマンド (新規レビュー実行) とは無関係。
    - **実装担当セッション (PR の作者) 内での自己発動禁止**。本コマンドは「別セッション独立レビュー」前提で、
      筆者と同じ context / session で起動すると「独立性」の趣旨が崩れる。
    - description のキーワード (「レビュー」等) がユーザー発言と一致しても、
      **ユーザーが `/review-pr <N>` と明示コマンドで呼ばない限り起動しない**。キーワード粗マッチは誤発動の元。
-->
