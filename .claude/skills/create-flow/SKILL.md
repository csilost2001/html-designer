...
### Rule 33: PostgreSQL aggregate (COUNT/SUM) の bigint 文字列化に CAST 必須 (#780 / retail 落とし穴 26)

- `SELECT COUNT(*) AS "<name>" FROM ...` / `SELECT SUM(<col>) AS "<name>" ...` で取得した値を後続 step で数値比較する場合、SQL 側で `CAST(... AS INTEGER)` を明示する
- pg クライアント (node-postgres) は bigint を **文字列として** JS に渡すため、`"10" >= 20` の暗黙変換に依存する形になり、特定 input でバグる
- 例: `SELECT CAST(COUNT(*) AS INTEGER) AS "itemCount" FROM cart_items WHERE cart_id = @cart.id`
- 将来 #781 (`outputBinding.transformations`) が schema に入れば runtime に任せられるが、現状は SQL 側で明示が必須
