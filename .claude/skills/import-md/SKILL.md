...
- `<project>/<dataDir>/generic-definitions/<kind>/*.json` 配下 (例: `examples/retail/harmony/generic-definitions/...`) は **AJV 対象外** だが最小 RFC メタモデル lint (`node scripts/spec-check/lint-generic-definitions.mjs <project-dir>`) で kind / required / path 整合を検証 (§10 (B) soft gate)
