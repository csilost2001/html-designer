 ...
 *
 * 注: ユーザー管理 (UserDetailsService / InMemoryUserDetailsManager / DB 連携 等) は
 * プロジェクト要件に応じて追加してください。本テンプレは認証フィルタチェーンのみ。
 *
...
##### 3. AuthController.java — logout 完全実装

ProcessFlow の `act-logout` action から生成される Controller は **Spring Security の `SecurityContextLogoutHandler` でセッション破棄** + redirect:
...
**注**: SecurityConfig の `.logout(...)` で同 path を登録しているため、Spring Security の `LogoutFilter` が先にセッション破棄を行う。本 Controller の実装は **Filter が無効化されたケース (テスト等) のためのフォールバック** として残す。Spring Security 推奨の運用では Controller を空にして Filter のみに任せる選択肢もある (この場合は AuthController クラスごと省略可)。
...
<出力先>/
  ...
  src/main/resources/templates/
    login.html
  # AuthController.java は ProcessFlow の act-logout から生成 (httpRoute.auth=required の act-logout が存在する場合)
```
