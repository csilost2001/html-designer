...
MCP ツール `mcp__designer-mcp__designer__workspace_status` または `workspace_inspect` で active workspace を特定し、
...
`designer/src/utils/techStackConstraints.ts` の `validateTechStackConstraints()` 相当チェックを実施する。
...
```bash
cd designer && npx tsc --noEmit --allowJs --checkJs 2>&1 | head -30
```
...
- `data/` ディレクトリを変更しない
- `designer/` / `designer-mcp/` のソースコードを変更しない
