...
各生成パスの詳細テンプレートは Phase B/C (別セッション) で追加予定です。本 Step では分岐ロジックと参照リンクを定義します。
...
```html
<!-- templates/<path>/<screenName>.html (layout decorate) -->
<!DOCTYPE html>
...
`layouts/<pageLayoutId>.html` は Step 3-D で生成するテンプレート (Phase B で追加)。
**Phase B/C 追加予定**: 詳細テンプレートは `.claude/skills/generate-code/templates/frontend/thymeleaf-bootstrap/LAYOUT.md` を参照 (Phase B で追加)。

#### NestJS/Next.js 系 (layout decorate モード)

```tsx
// app/<path>/page.tsx (layout decorate)
import { AppLayout } from '@/app/layouts/<pageLayoutId>';
...
`AppLayout` は Step 3-D で生成するコンポーネント (Phase C で追加)。
**Phase B/C 追加予定**: 詳細テンプレートは `.claude/skills/generate-code/templates/frontend/react-tailwind-next/LAYOUT.md` を参照 (Phase C で追加)。
...
**Phase B/C 追加予定**: 詳細テンプレートは以下を参照:
- Thymeleaf 系: `.claude/skills/generate-code/templates/frontend/thymeleaf-bootstrap/FRAGMENT.md` (Phase B で追加)
- NestJS/Next.js 系: `.claude/skills/generate-code/templates/frontend/react-tailwind-next/COMPONENT.md` (Phase C で追加)
...
```
<出力先>/
  app/components/gadgets/<gadget-id>/
    index.tsx                    ('use client' component)
    actions.ts                   ('use server' actions、processFlowId 連携あり時のみ)
```
...
import { <gadgetActionName>Action } from './actions';

interface <GadgetName>GadgetProps {
  // items[direction=output] の valueFrom → prop 型
  initialData?: Record<string, unknown>;
}

export function <GadgetName>Gadget({ initialData }: <GadgetName>GadgetProps) {
  // items[direction=input] → useState
  const [formState, setFormState] = useState({});

  return (
    <div className="gadget-<gadget-id>">
      {/* items[] → input/output 要素 (Tailwind クラス) */}
      {/* processFlowId があれば <form action={<gadgetActionName>Action}> */}
    </div>
  );
}
```

#### Server Actions テンプレート (processFlowId 連携あり)
...
// Spec anchor: Gadget <gadget-id> act-<actionId>
export async function <gadgetActionName>Action(formData: FormData) {
  const payload = {
    // inputs[] → formData.get('<name>')
  };
  const res = await fetch('<httpRoute.path>', {
    method: '<httpRoute.method>',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
  if (!res.ok) throw new Error('Gadget action failed');
...
**Phase B/C 追加予定**: 詳細テンプレートは以下を参照:
- Thymeleaf 系: `.claude/skills/generate-code/templates/frontend/thymeleaf-bootstrap/LAYOUT.md` (Phase B で追加)
- NestJS/Next.js 系: `.claude/skills/generate-code/templates/frontend/react-tailwind-next/LAYOUT.md` (Phase C で追加)
...
<body>
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <!-- region: main (Page の body-content を inject) -->
      <main class="col" th:fragment="layout-content(content)">
...
// PageLayout: <pageLayoutId> (<pageLayout.name>)
// regions: <region-ids.join(', ')>
import { <HeaderGadgetName>Gadget } from '@/app/components/gadgets/<headerGadgetId>';
import { <SidebarGadgetName>Gadget } from '@/app/components/gadgets/<sidebarGadgetId>';
import { <FooterGadgetName>Gadget } from '@/app/components/gadgets/<footerGadgetId>';
...
.claude/skills/generate-code/templates/
  ...
  frontend/thymeleaf-bootstrap/
    ...
    LAYOUT.md     — PageLayout passive layout 生成ルール (Step 3-D 用、Phase B で追加予定)
    FRAGMENT.md   — Gadget fragment + GadgetController 生成ルール (Step 3-C 用、Phase B で追加予定)
  frontend/react-tailwind-next/
    PAGE.md       — Next.js App Router ページ生成ルール (kind 別パターン)
    LAYOUT.md     — AppLayout component 生成ルール (Step 3-D 用、Phase C で追加予定)
    COMPONENT.md  — Gadget client component + server actions 生成ルール (Step 3-C 用、Phase C で追加予定)
```
...
frontend (React + Next.js):
  ...
  <出力先>/components/<domain>/
    <ComponentName>.tsx
  # PageLayout (Step 3-D):
  <出力先>/app/layouts/<pageLayoutId>/
    index.tsx
  # Gadget (Step 3-C):
  <出力先>/app/components/gadgets/<gadgetId>/
    index.tsx
    actions.ts  (processFlowId あり時のみ)
```
