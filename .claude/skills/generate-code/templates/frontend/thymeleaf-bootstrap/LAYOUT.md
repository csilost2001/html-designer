...
全テンプレート (LAYOUT.md / FRAGMENT.md / PAGE.md) で一貫して使用する Bootstrap クラス。固定 4 region に加え、`page-layout.v3.schema.json` の region 命名 (`^[a-z][a-zA-Z0-9_-]*$`) で許容される **任意 region の動的展開規約**も含む (#1035 F + S-2)。

### 固定 region (header / sidebar / main / footer)
...
| **未知 region** (fallback) | `<div role="region" aria-label="<regionName>" class="region-<regionName>">` | header と main の間に挿入。コメントで「TODO: <regionName> の class を必要に応じて調整してください」と注記 |

**展開順序**: `PageLayout.regions[].order` 昇順で出力。固定 region (header/sidebar/main/footer) は固定スロット、任意 region は header < x < main < footer の範囲に order 順で挿入。順序が曖昧な場合は header 直下にフォールバック配置。

**assignment 解決ルール** (任意 region も同様):
- `assignments[<regionName>]` に gadget ID あり → 上記タグ内に `<div th:replace="~{fragments/<gadgetId> :: gadget}"></div>` を生成
- assignment なし → 「region が一部未割り当ての場合」セクションの placeholder コメントを出力
...
- 任意 region (breadcrumb / subHeader / toolbar 等) は header と main row の間、または main row 内の適切な位置に挿入

### 任意 region を含む完成例 (breadcrumb + notification + 固定 4 region)

```html
<body class="d-flex flex-column min-vh-100">
  <!-- region: header (assignment → gadget) -->
  <nav class="navbar navbar-expand bg-primary navbar-dark px-3">
    <div th:replace="~{fragments/<<headerGadgetId>> :: gadget}"></div>
  </nav>

  <!-- region: breadcrumb (任意、assignment → gadget or content placeholder) -->
  <nav class="breadcrumb-nav mb-2" aria-label="breadcrumb">
    <div th:replace="~{fragments/<<breadcrumbGadgetId>> :: gadget}"></div>
  </nav>

  <!-- region: notification (任意、絶対配置、assignment → gadget) -->
  <div class="position-fixed top-0 end-0 p-3" style="z-index: 1080">
    <div th:replace="~{fragments/<<notificationGadgetId>> :: gadget}"></div>
  </div>
  ...
  <div class="container-fluid flex-grow-1">
    <div class="row h-100">
      ...
      <main class="col p-4" layout:fragment="layout-content"></main>
    </div>
  </div>
