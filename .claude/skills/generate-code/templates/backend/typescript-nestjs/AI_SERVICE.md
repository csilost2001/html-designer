...
export class AiRuntimeService {
  ...
  private normalizeAndValidate(
    ...
    if (kind === 'structuredObject' && responseFormat?.schema) {
      // AJV や zod 等で schema validate (本テンプレートでは型のみ示す)
      // const ok = ajv.validate(responseFormat.schema, raw.object);
      // if (!ok) throw new HttpException({ code:'AI_RESPONSE_FORMAT_VIOLATION', ... }, 502);
    }
    return raw;
  }
