 ...
 *
 * PLACEHOLDER:
 *   ${AI_RUNTIME_SERVICE_TYPE}   — Phase 2-C で確定する AI runtime service クラス (例: AiRuntimeService)
 *   ${AI_RUNTIME_SERVICE_METHOD} — invoke / execute 等 (Phase 2-C で確定)
 *   ${AI_RUNTIME_IMPORT_PATH}    — 実 import パス (Phase 2-C で確定)
 *
 * 旧 mocks/claude-api.ts (Anthropic 形式 HTTP レスポンス mock) は Phase 2-A / 2-B 移行で廃止。
 */
import type { ${AI_RUNTIME_SERVICE_TYPE} } from '${AI_RUNTIME_IMPORT_PATH}';
...
export function mockAiText(
  ...
  return jest.spyOn(svc, '${AI_RUNTIME_SERVICE_METHOD}').mockResolvedValue(result);
}
...
export function mockAiFailure(
  ...
  return jest
    .spyOn(svc, '${AI_RUNTIME_SERVICE_METHOD}')
...
```typescript
/**
 * AI flow テスト: ${FLOW_NAME} mock mode (Phase 2-B)
 *
 * kind=${AI_STEP_KIND} step を AI runtime service ($PLACEHOLDER) で stub し、
...
### 16-E. AI テスト PLACEHOLDER 置換チートシート (Phase 2-B)
...
| `${AI_RUNTIME_SERVICE_TYPE}` | Phase 2-C で確定 (NestJS の AI runtime service クラス名) | `AiRuntimeService` |
| `${AI_RUNTIME_SERVICE_METHOD}` | Phase 2-C で確定 (invoke / execute 等) | `invoke` |
| `${AI_RUNTIME_IMPORT_PATH}` | Phase 2-C で確定 (実 import パス) | `<placeholder>` |
