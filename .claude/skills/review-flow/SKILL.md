...
| 17. PostgreSQL aggregate 型変換 (#780) | (なし、機械検出困難) | `COUNT(*)` / `SUM(...)` に `CAST AS INTEGER` 等の型キャストがあるか / pg クライアント bigint 文字列化への対処 |
...
#### 観点 17: PostgreSQL aggregate 型変換 (#780 / 落とし穴 26)

`COUNT(*)` / `SUM(...)` の戻り値型が PG では bigint で pg クライアント経由で文字列化される問題を検証する。

- `SELECT COUNT(*) AS "<name>" FROM ...` / `SELECT SUM(<col>) AS "<name>" ...` で取得した値を後続 step で数値比較しているか確認
- pg クライアント (node-postgres) は bigint を **文字列として** JS に渡すため、`"10" >= 20` の暗黙変換に依存する形になる
- 修正方針: SQL 側で `CAST(COUNT(*) AS INTEGER)` / `CAST(SUM(...) AS INTEGER)` を明示する。または #781 (`outputBinding.transformations`) が schema に入れば runtime に任せる
- 検出手順:
  1. `grep -P "COUNT\\(|SUM\\(" flow.json` で aggregate を抽出
  2. `CAST` または `::integer` キャストがなければ Should-fix
  3. 後続 step での数値比較 (`>=` / `<` / arithmetic) があるかも合わせて確認
