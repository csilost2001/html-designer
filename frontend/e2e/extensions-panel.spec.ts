...
test.describe("拡張管理 UI (#447)", () => {
  ...
  // TODO(#945 follow-up): backend `projectStorage.ts:140` が存在しない
  ...
  // schema 検証 fail。backend 側コードを v3 schema 参照に修正必要。
  test.skip("レスポンス型を追加して保存できる (#945 follow-up: backend extensions schema 参照修正待ち)", async ({ page }) => {
