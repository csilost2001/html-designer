 ...
 * 前提: backend サーバーが ws://localhost:5179 で起動済みであること
 *
 * プロトコルの向き:
...
// TODO(#945-G follow-up): sendBrowserRequest が短命 WS で都度 clientId を発行する
...
// 構造に書き直す必要あり。本 ISSUE スコープ外、別 follow-up で対応。
test.describe.skip("wsBridge ファイル操作 (#945-G follow-up: sendBrowserRequest 構造修正待ち)", () => {
   ...
   * backend が古いビルドで起動していると「未知のメソッド」エラーが返るため、
  ...
  test.describe("getFileMtime", () => {
    test.beforeEach(async () => {
      ...
      if (!supported) test.skip(true, "backend に getFileMtime が実装されていません。再起動してください");
    });
